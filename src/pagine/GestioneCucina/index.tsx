...
import { CheckCircle, Clock, Loader2, Utensils, ClipboardList, Wine, X } from 'lucide-react';
    ...
    const fetchOrders = async (_restId: string) => {
        ...
        if (activeOrders) {
            const displayOrders: DisplayOrder[] = activeOrders.map((o: any) => {
                ...
                const kitchenItems = Object.values(groupedItemsMap);
            ...
            <div className="flex-1 flex overflow-hidden mt-16 sm:mt-20">
                ...
                <SidebarContainer className="w-full md:w-80 border-r md:border-gray-200 flex flex-col shadow-lg shrink-0 z-20 bg-white">
                    <div className="p-4 border-b border-gray-200 bg-gray-50 flex justify-between items-center">
                        <h2 className="font-bold text-gray-800 text-lg">Comande In Arrivo</h2>
                ...
                {/* Main Content: Dettaglio Comanda Selezionata (Desktop) */}
                <div className="hidden md:flex flex-col flex-1 overflow-y-auto p-8 bg-gray-100 items-center">
                    {selectedOrder ? (
                                ...
                                <div className="p-6 border-b border-gray-100 bg-white flex justify-between items-center">
                                    <div>
                                        ...
                                        <div className="flex items-center gap-2 mt-1 text-gray-500 font-medium">
                                            ...
                                            Ricevuta {Math.floor((Date.now() - selectedOrder.timestamp) / 60000)} minuti fa
                                        </div>
                                    </div>
                                            ...
                                            >
                                                <div className="flex items-center gap-6">
                                                    <div className={`w-12 h-12 rounded-full flex items-center justify-center font-black text-xl ${
                                                        item.status === 'served' ? 'bg-gray-200 text-gray-500' : 'bg-orange-100 text-orange-700'
                                                    }`}>
                                                        {item.quantity}
                                                    </div>
                                                    <div>
                        ...
                        <div className="flex flex-col items-center justify-center h-full text-gray-400">
                            ...
                            <p className="text-xl font-medium">Seleziona una comanda dalla lista</p>
                        </div>
                    )}
                </div>
            </div>
                                ...
                                >
                                    <div className="flex items-center gap-4">
                                        <div className={`w-10 h-10 rounded-full flex items-center justify-center font-bold ${
                                            item.status === 'served' ? 'bg-gray-200' : 'bg-orange-100 text-orange-700'
                                        }`}>
                                            {item.quantity}
                                        </div>
                                        <div>
